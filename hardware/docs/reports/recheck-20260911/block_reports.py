...
out=Path('output/pdf/bloklar-20260911');out.mkdir(parents=True,exist_ok=True)
root=E.parse('reports/recheck-20260911/after.xml').getroot();cs={c.get('ref'):c for c in root.find('components')};nets={};func={}
...
blocks=[
 ...
 'Q5 AO3400A kullanıcı seçimi korundu. G1 PWM/100 Ω ve100 kΩ pull-down; S2 GND; D3 BL_SINK. Açık kalan drain yeniden bağlandı. Eksik AO3400A footprint kütüphanesi yerine üretici SOT23 paketine uygun standart Package_TO_SOT_SMD:SOT-23 atandı (G1/S2/D3).',
 ...
 dict(file='09_POWER_OUTPUT',title='Güç çıkışı ve panel bağlantıları',sheet='POWER OUTPUT',refs=['J4','J5','J6'],pins=['J4','J5','J6'],source='datasheets/pi-CCS-JOHN-108-0904-001.pdf; datasheets/DG142R-5.08-XXP-1Y-00A(H)T0-l.pdf',notes=[
 'J4.1 OUT_POS; J4.2 GND. J5 kırmızı108-0902-001 OUT_POS’a; J6 siyah108-0903-001 GND’ye kabloyla bağlanır. Kullanıcı kararıyla yalnız iki jack vardır; J7 yoktur. Siyah jack protective earth değil, devreGND’sidir.',
...
def footer(c,d):c.setFont('Arial',8);c.drawString(43,24,'11.09.2026 • Şematik denetimi • Fiziksel PCB ve prototip doğrulaması yapılmadı');c.drawRightString(550,24,str(d.page))
...
stats=Path('reports/recheck-20260911/erc.rpt').read_text(encoding='utf8');errors=stats.count('; error');warnings=stats.count('; warning')
for b in blocks:
 ...
 im=Path('reports/recheck-20260911/svg')/('masaüstü güç kaynağı-'+b['sheet']+'.png')
 st.append(Image(str(im),width=509,height=360));st.append(Spacer(1,10))
 st.append(para(f'Tüm projede ERC: {errors} hata, {warnings} uyarı. Beş hata power_pin_not_driven sınıfındadır. Uyarılar:323 off-grid,6 açık tel ucu,1 kaynaksız BACKLIGHT_3V0,1 encoder kütüphane farkı. Tam konumlar teslim paketindeki ERC raporundadır. Off-grid uyarıları telin kopuk olduğu anlamına gelmez. Gerçek bağlantılar netlistten kontrol edildi.'))
...
before=E.parse('reports/recheck-20260911/before.xml').getroot();old={c.get('ref'):c for c in before.find('components')};delta=[]
...
changes=json.loads(Path('reports/recheck-20260911/changes.json').read_text(encoding='utf8'));lr=[v for v in changes if v[2]=='Label direction'];st.append(tab([['Sayfa','Net','Önce','Sonra']]+[[v[0],v[1],v[3],v[4]] for v in lr],[156,177,88,88]))
...
index=['# Tasarım dokümanları','', '11 Eylül 2026 — Güncel teslim paketi','']
...
Path('output/TASARIM_RAPORLARI_20260911.md').write_text('\n'.join(index),encoding='utf8')
